...
static int blob_dup(stride_blob_t *dst, const stride_blob_t *src) {
    dst->data = NULL;
    dst->bit_len = src ? src->bit_len : 0;
    if (!src || !src->data || src->bit_len == 0) {
        return 0;
    }
...
static int seg_find_fwd(const unsigned char *seg, size_t stride, size_t total,
                        size_t from, const stride_blob_t *target, size_t *hit) {
    if (!target || target->bit_len == 0 || target->bit_len % stride != 0) {
        return 0;
    }
    size_t ds = target->bit_len / stride;
    for (size_t k = from; k + ds <= total; k++) {
        if (bits_eq(seg, k * stride, target->data, target->bit_len)) {
            *hit = k;
            return 1;
        }
    }
...
static int seg_find_rev(const unsigned char *seg, size_t stride, size_t total,
                        size_t from, const stride_blob_t *target, size_t *hit) {
    if (total == 0 || !target || target->bit_len == 0 ||
        target->bit_len % stride != 0) {
        return 0;
    }
    size_t ds = target->bit_len / stride;
    ...
    for (;;) {
        if (k + ds <= total &&
            bits_eq(seg, k * stride, target->data, target->bit_len)) {
            *hit = k;
            return 1;
        }
        if (k == 0) {
...
static int move_try_merge(stride_step_t *n, stride_move_t kind, size_t value) {
    switch (n->move) {
        ...
        case STRIDE_MOVE_SKIP_BITS:
            if (kind == STRIDE_MOVE_SKIP_BITS) {
                n->move_value += value;
                return 1;
            }
            return 0;
...
int stride_seq_step_fwd(stride_seq_t *seq, size_t steps) {
    return seq_add_move(seq, STRIDE_MOVE_STEP_FWD, steps, NULL);
}
int stride_seq_step_back(stride_seq_t *seq, size_t steps) {
    return seq_add_move(seq, STRIDE_MOVE_STEP_BACK, steps, NULL);
}
int stride_seq_abs_head(stride_seq_t *seq, size_t steps) {
    return seq_add_move(seq, STRIDE_MOVE_ABS_HEAD, steps, NULL);
}
int stride_seq_abs_end(stride_seq_t *seq, size_t steps) {
    return seq_add_move(seq, STRIDE_MOVE_ABS_END, steps, NULL);
}
int stride_seq_skip_bits(stride_seq_t *seq, size_t bit_len) {
    return seq_add_move(seq, STRIDE_MOVE_SKIP_BITS, bit_len, NULL);
}
...
int stride_seq_capture_steps(stride_seq_t *seq, size_t steps) {
    return seq_add_act(seq, STRIDE_ACT_CAPTURE_STEPS, NULL, steps);
}
...
int stride_seq_run(const stride_seq_t *seq, size_t stride, const void *segment,
    ...
    for (const stride_step_t *n = seq->head; n; n = n->next, i++) {
        ...
        switch (n->move) {
            ...
            case STRIDE_MOVE_SKIP_BITS: {
                if (n->move_value % stride != 0) {
                    goto fail;
                }
                size_t s = n->move_value / stride;
                if (s > total - pos) {
                    goto fail;
                }
                pos += s;
            ...
            case STRIDE_MOVE_FIND_REV: {
                size_t hit;
                int found = (n->move == STRIDE_MOVE_FIND_FWD)
                                ? seg_find_fwd(seg, stride, total, pos,
                                               &n->move_target, &hit)
                                : seg_find_rev(seg, stride, total, pos,
                                               &n->move_target, &hit);
                if (!found) {
                    goto fail;
                }
        ...
        switch (n->act) {
            ...
            case STRIDE_ACT_COMPARE: {
                if (n->act_target.bit_len % stride != 0) {
                    goto fail;
                }
                size_t es = n->act_target.bit_len / stride;
                if (es > total - pos) {
                    goto fail;
                }
                if (!bits_eq(seg, pos * stride, n->act_target.data,
                             n->act_target.bit_len)) {
                    goto fail;
                }
                pos += es;
                break;
            ...
            case STRIDE_ACT_CAPTURE_STEPS: {
                if (!params || n->act_value > total - pos) {
                    goto fail;
                }
                size_t start_bit = pos * stride;
                if (start_bit % 8 != 0 || out_idx >= param_capacity) {
                    goto fail;
                }
            ...
            case STRIDE_ACT_CAPTURE_UNTIL: {
                ...
                size_t end = total;
                size_t hit;
                if (seg_find_fwd(seg, stride, total, start, &n->act_target,
                                 &hit)) {
                    end = hit;
                }
